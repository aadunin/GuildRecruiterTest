...
local function GRU(cmd)
  if SlashCmdList and SlashCmdList["GRU"] then
    SlashCmdList["GRU"](cmd or "")
  else
...
local function anyPrinted(pat)
  for _, m in ipairs(T._printed) do
    if type(m) == "string" and m:match(pat) then return true end
  end
  return false
end
...
T.describe("Smoke — загрузка и базовый статус", function()
  T.it("Аддон загружен и /gru доступен", function(t)
    resetState()
    t.assertTrue(type(SlashCmdList["GRU"]) == "function", "/gru зарегистрирован")
  end)
...
T.describe("Шаблоны и рандомизация", function()
  ...
  T.it("Защита от дубликатов при addtmpl", function(t)
    ...
    GRU("addtmpl X")
    GRU("addtmpl X")
    t.assertTrue(anyPrinted("уже есть"), "Предупреждение о дубликате")
  end)
end)
...
T.describe("Отправка и список каналов", function()
  ...
  T.it("listchannels показывает список или предупреждает об отсутствии", function(t)
    resetState()
    GRU("listchannels")
    local ok = anyPrinted("Список каналов:") or anyPrinted("Вы не подключены")
    t.assertTrue(ok, "Либо список, либо предупреждение")
  end)
end)
